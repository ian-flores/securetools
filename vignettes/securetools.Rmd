...
at all. The `tool_query_sql()` exposes a structured interface where
...
memory and crash the host process. The `tool_data_profile()` enforces
...
The `tool_r_help()` gives safe access to R documentation:
...
tools (like `tool_fetch_url()`) also support `max_calls_per_minute` for
...
| Write File | `tool_write_file()` | Path scoping, size limits, overwrite protection |
