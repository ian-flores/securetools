...
repeats until it has an answer. Pass securetools to the `agent()` constructor
with `secure = TRUE` so tool calls run inside a securer sandbox.
...
Agent loops can generate many tool calls. Use `max_calls` on tool factories
to set a hard cap that prevents runaway execution.
...
You can combine securetools factories with custom `securer::securer_tool()`
definitions in the same agent. All tools run inside the same secure session.
