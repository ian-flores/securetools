...
securetools is one of four packages for building governed AI agents in R:
...
sessions, and (via `guarded_tool()`) wraps them with secureguard
guardrails, giving agents safe access to files, SQL, URLs, and
computation.
...
[ellmer](https://ellmer.tidyverse.org/)'s native OpenTelemetry support to
trace the LLM side of your agent, and
...
- `vignette("data-analyst-agent")` -- Building a governed data analyst
  agent with securer + secureguard + securetools
- `system.file("examples", package = "securetools")` -- Runnable examples,
  including a Plumber REST API with guarded chat and sandboxed execution
